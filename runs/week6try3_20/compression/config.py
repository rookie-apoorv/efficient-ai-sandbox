...
# --------------------------------------------------------------------------
# ---- 20% TARGET ----
# 20% of float16 is 3.2 bits/weight, below the int4 floor, so the 40% mix of
# int4/int8 has no headroom here. The planner inverts: int2 is the default and
# int4 is what the budget buys back. Group 128 rather than 64 because the
# codebook overhead (24 bits per group) is a much larger share of the total at
# 2 bits -- halving it frees roughly 10% more weights for int4 promotion, which
# matters more than the finer grid.
GROUP_SIZE = 128
BASE_BITS = 2            # default bit width
HIGH_BITS = 4            # bit width for tensors the planner promotes
