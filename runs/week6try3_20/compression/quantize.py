...
Storage cost per weight: ``bits + 16/group + 8/group`` bits
    ...
    2-bit, group 128 -> 2.1875 bits/weight (13.7% of float16)
    4-bit, group 128 -> 4.1875 bits/weight (26.2% of float16)
    4-bit, group  64 -> 4.375  bits/weight (27.3% of float16)
    8-bit, group  64 -> 8.375  bits/weight (52.3% of float16)

The 2-bit width exists for the 20% and 10% targets, where the 40% mix of
int4/int8 has no headroom: 20% of float16 is 3.2 bits/weight, below the int4
floor, so the planner must choose between int2 and int4 instead.
"""
...
# Codes per byte. Both widths divide 8 evenly, so packing stays
# byte-aligned and no code ever straddles a byte boundary.
...
def pack_codes(codes: torch.Tensor, bits: int) -> torch.Tensor:
    """Pack sub-byte codes, least-significant field first.

    At 2 bits four codes share a byte, at 4 bits two do, at 8 bits none.
    Columns are padded up to a whole number of bytes; the padding is discarded
    on unpack using the true column count from the metadata.
    """
    if bits == 8:
        return codes.contiguous()
    per = CODES_PER_BYTE[bits]
    remainder = codes.shape[1] % per
    if remainder:
        codes = torch.nn.functional.pad(codes, (0, per - remainder))
    out = codes[:, 0::per].clone()
    for k in range(1, per):
        out |= codes[:, k::per] << (bits * k)
...
def quantized_nbytes(rows: int, cols: int, bits: int, group_size: int) -> int:
    ng = n_groups_for(cols, group_size)
    per = CODES_PER_BYTE[bits]
    code_bytes = rows * ((cols + per - 1) // per)
